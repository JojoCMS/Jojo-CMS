...
{foreach from=$jojo_articles item=a key=k}
    ...
    {if $k<1 || $a.weighting==0 || $a.snippet=='full'}
        ...
        <div class="media-body">
            <h3 class="media-heading">{if $a.snippet=='full'|| strlen($a.bodyplain)<$a.snippet}{$a.title}{else}<a href="{$SITEURL}/{$a.url}" title="{$a.title}">{$a.title}</a>{/if}</h3>
            {if $a.subtitle}<h4>{$a.subtitle}</h4>{/if}
            {if $a.snippet=='full' || strlen($a.bodyplain)<$a.snippet}{$a.ar_body}{else}<p>{$a.bodyplain|truncate:$a.snippet} <a href="{$SITEURL}/{$a.url}" title="{$a.title}" class="more">{$a.readmore}</a></p>{/if}
