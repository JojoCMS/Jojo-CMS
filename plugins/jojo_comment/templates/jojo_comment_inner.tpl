        ...
        {else}
        <h4>{if $c.website}<a href="#comments" rel="nofollow" onclick="redirectSubmit('{$c.website}'); return false;" title="{$c.website}">{$c.name}</a>{else}{$c.name}{/if}<span class="date"> - {$c.timestamp|date_format}</span></h4>
        {/if}
