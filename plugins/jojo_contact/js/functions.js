...
function preFlight(formData, jqForm, options) {
    ...
    var formName = $('h1').text() + ' ' + $('#form' + formID).attr('name');
    if (typeof ga !== 'undefined') {
        ga('send', 'event', formName + ' id:form' + formID, 'submit');
...
function showFormTab(formid, tabid) {
    ...
    if (typeof ga !== 'undefined') {
        ga('send', 'event', formName, tabName);
    } else if (typeof _gaq !== 'undefined') {
...
$(document).on('change', '.contact-form .btn-file :file', function() {
  var input = $(this),
...
$(document).ready( function() {
    $('.contact-form .btn-file :file').on('fileselect', function(event, numFiles, label) {
