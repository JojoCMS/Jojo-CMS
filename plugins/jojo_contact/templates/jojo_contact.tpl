...
{foreach from=$fields key=k item=f }{assign var=x value=`$k-1`}
    ...
    {if !($f.type=='heading' || $f.type=='note')}<label for="{$f.field}" class="control-label{if !$f.showlabel} sr-only{/if}{if $f.required} required" title="{$f.display} ##is a required field##{/if}">{$f.display}</label>{/if}
    {/if}{if $f.type == 'hidden'}<input type="hidden" name="{$f.field}" id="form{$form.form_id}-{$f.field}" value="{$f.value}" />
    {elseif $f.type == 'textarea'}<textarea class="form-control input textarea{if $f.class} {$f.class}{/if}{if $f.required} required{/if}" rows="{$f.rows|default:'10'}" cols="{$f.cols|default:'29'}" {if $f.maxlength}maxlength="{$f.maxlength}" {/if}name="{$f.field}" id="form{$form.form_id}-{$f.field}"{if $f.placeholder} placeholder="{$f.placeholder}"{/if}>{$f.value}</textarea>
    {elseif $f.type == 'checkboxes'}{foreach from=$f.options key=ck item=fo}<div class="checkbox{if $f.class} {$f.class}{/if}"><label for="{$f.field}_{$ck}" title="{$f.display}"><input type="checkbox"{if $f.required} class="required"{/if} name="{$f.field}[]" id="form{$form.form_id}-{$f.field}_{$ck}" value="{$fo}"{if $f.valuearr}{foreach from=$f.valuearr item=fa}{if $fa==$fo} checked="checked"{/if}{/foreach}{elseif $f.value == 'checked'} checked="checked"{/if} />{$fo}</label></div>
            {/foreach}
    {elseif $f.type == 'radio'}{foreach from=$f.options key=rk item=button}<div class="radio{if $f.class} {$f.class}{/if}"><label for="{$f.field}_{$rk}" title="{$f.display}"><input type="radio"{if $f.required} class="required"{/if} name="{$f.field}" id="form{$form.form_id}-{$f.field}_{$rk}" value="{$button}" {if $f.value == $button} checked="checked"{/if} />{$button}</label></div>
            {/foreach}
    {elseif $f.type=='select'}<select name="{$f.field}" id="form{$form.form_id}-{$f.field}" class="form-control{if $f.required} required{/if}">
              <option value="">Select</option>
                {foreach from=$f.options item=so}<option value="{$so}"{if $f.value == $so} selected="selected"{/if}>{$so}</option>
                {/foreach}
        </select>
    {elseif $f.type=='list'}<select name="{$f.field}[]" id="form{$form.form_id}-{$f.field}" multiple="multiple" class="form-control{if $f.required} required{/if}">
    ...
    <div class="input-group">
                <span class="input-group-btn">
                    <span class="btn btn-primary btn-file">
                        Browse&hellip; <input type="file" class="fileupload {$f.type}{if $f.class} {$f.class}{/if}{if $f.required} required{/if}" name="FILE_{$f.field}[]" id="FILE_form{$form.form_id}-{$f.field}""{if $f.size} size="{$f.size}"{/if} multiple>
                    </span>
                </span>
                <input type="text" class="form-control" title="Files" readonly>
    </div>
    {elseif $f.type=='emailwithconfirmation'}<input type="email" class="form-control input text{if $f.class} {$f.class}{/if}{if $f.required} required{/if}" size="{$f.size}" name="{$f.field}" id="form{$form.form_id}-{$f.field}" value=""{if $f.placeholder} placeholder="{$f.placeholder}"{/if} />
    {elseif $f.type=='date'}{if $anytime}<div class="input-group">{/if}
        <input type="{if $anytime}text{else}date{/if}" class="form-control input {$f.type}{if $f.class} {$f.class}{/if}{if $f.required} required{/if}{if $anytime} anytime{/if}{if $f.validation && $f.validation!=$f.type} {$f.validation}{/if}" {if $f.size}size="{$f.size}" {/if}name="{$f.field}" id="form{$form.form_id}-{$f.field}" value="{$f.value}"{if $f.placeholder} placeholder="{$f.placeholder}"{/if}{if $anytime && $f.options} data-format="{$f.options[0]}"{/if} />
    ...
    <div class="input-group">
        {/if}{if $f.prependvalue}<span class="input-group-addon">{$f.prependvalue}</span>{/if}<input type="{if $f.validation=='email'}email{elseif $f.validation=='url'}url{elseif $f.validation=='integer'}number{else}text{/if}" class="form-control input {$f.type}{if $f.class} {$f.class}{/if}{if $f.required} required{/if}{if $f.validation && $f.validation!=$f.type} {$f.validation}{/if}" {if $f.size}size="{$f.size}" {/if}{if $f.maxlength}maxlength="{$f.maxlength}" {/if}name="{$f.field}" id="form{$form.form_id}-{$f.field}" value="{$f.value}"{if $f.placeholder} placeholder="{$f.placeholder}"{/if} />{if $f.appendvalue}<span class="input-group-addon">{$f.appendvalue}</span>{/if}{if $f.prependvalue || $f.appendvalue}
    </div>{/if}
    {/if}
    ...
    <div class="form-fieldset text form-group">
        ...
        <input type="email" class="form-control input text{if $f.class} {$f.class}{/if}" {if $f.size}size="{$f.size}" {/if}name="{$f.field}_confirmation" id="form{$form.form_id}-{$f.field}_confirmation" value="{$f.value}"{if $f.placeholder} placeholder="{$f.placeholder}"{/if} />
    </div>
