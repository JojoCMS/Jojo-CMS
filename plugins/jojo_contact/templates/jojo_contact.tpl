...
{foreach from=$fields key=k item=f }{assign var=x value=`$k-1`}
    ...
    {elseif $f.type == 'textarea'}<textarea class="form-control input textarea{if $f.class} {$f.class}{/if}{if $f.required} required{/if}" rows="{$f.rows|default:'10'}" cols="{$f.cols|default:'29'}" {if $f.maxlength}maxlength="{$f.maxlength}" {/if}name="{$f.field}" id="{$f.field}" title="{$f.display}"{if $f.placeholder} placeholder="{$f.placeholder}"{/if}>{$f.value}</textarea>
    ...
    {elseif $f.type=='list'}<select name="{$f.field}[]" id="{$f.field}" multiple="multiple" class="form-control{if $f.required} required{/if}" title="{$f.display}">
    ...
    <div class="input-group">
                <span class="input-group-btn">
                    <span class="btn btn-primary btn-file">
                        Browse&hellip; <input type="file" class="fileupload {$f.type}{if $f.class} {$f.class}{/if}{if $f.required} required{/if}" name="FILE_{$f.field}[]" id="FILE_{$f.field}" title="{$f.display}"{if $f.size} size="{$f.size}"{/if} multiple>
                    </span>
                </span>
    ...
    <div class="input-group">
        {/if}{if $f.prependvalue}<span class="input-group-addon">{$f.prependvalue}</span>{/if}<input type="{if $f.validation=='email'}email{elseif $f.validation=='url'}url{elseif $f.validation=='integer'}number{else}text{/if}" class="form-control input {$f.type}{if $f.class} {$f.class}{/if}{if $f.required} required{/if}{if $f.validation && $f.validation!=$f.type} {$f.validation}{/if}" {if $f.size}size="{$f.size}" {/if}{if $f.maxlength}maxlength="{$f.maxlength}" {/if}name="{$f.field}" id="{$f.field}" value="{$f.value}"{if $f.placeholder} placeholder="{$f.placeholder}"{/if} title="{$f.display}" />{if $f.appendvalue}<span class="input-group-addon">{$f.appendvalue}</span>{/if}{if $f.prependvalue || $f.appendvalue}
    </div>{/if}
    {/if}
