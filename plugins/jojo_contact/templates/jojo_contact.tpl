...
{foreach from=$fields key=k item=f }{assign var=x value=`$k-1`}
    ...
    {elseif $f.type == 'checkboxes'}<div class="form-field">
            {foreach from=$f.options key=ck item=fo}<div class="checkbox"><input type="checkbox" class="checkbox{if $f.class} {$f.class}{/if}{if $f.required} required{/if}" name="{$f.field}[{$ck}]" id="{$f.field}_{$ck}" value="{$fo}"{foreach from=$f.valuearr item=fa}{if $fa==$fo} checked="checked"{/if}{/foreach} /><label for="{$f.field}_{$ck}">{$fo}</label></div>
