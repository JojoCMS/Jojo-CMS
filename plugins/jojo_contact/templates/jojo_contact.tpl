...
{foreach from=$fields key=k item=f }{assign var=x value=`$k-1`}
    ...
    {elseif $f.type == 'checkboxes'}<div class="form-field">
            {foreach from=$f.options key=k item=fo }<div class="checkbox"><input type="checkbox" class="checkbox{if $f.class} {$f.class}{/if}{if $f.required} required{/if}" name="{$f.field}[{$k}]" id="{$f.field}_{$k}" value="{$fo}"{foreach from=$f.valuearr item=fa}{if $fa==$fo} checked="checked"{/if}{/foreach} /><label for="{$f.field}_{$k}">{$fo}</label></div>
            {/foreach}{if $f.required}<span class="required">*</span>{/if}
            </div>
    {elseif $f.type == 'radio'}<div class="form-field">
            {foreach from=$f.options key=k item=button }<input type="radio" class="radio{if $f.class} {$f.class}{/if}{if $f.required} required{/if} name="{$f.field}" id="{$f.field}_{$k}" value="{$button}" {if $f.value == $button} checked="checked"{/if} /><label for="{$f.field}_{$k}"> {$button}</label>
            {/foreach}{if $f.required}<span class="required">*</span>{/if}
        </div>
