...
{foreach from=$fields key=k item=f }{assign var=x value=`$k-1`}
    ...
    {/if}{if !in_array($f.type,array('heading','note')) && $f.type!='hidden'}<div class="form-fieldset {if $f.type == 'emailwithconfirmation'}text{else}{$f.type}{/if}{if $f.class} {$f.class}{/if}">
        {if $f.showlabel}<label for="{$f.field}">{if $f.display}{$f.display}:{/if}{if $f.required} <span class="required">*</span>{/if}</label>
        {/if}
...
{/foreach}
    ...
    {/if}
        <div  class="form-fieldset submit">
            {if $form.form_submit_label}<label>&nbsp;</label>{/if}<input type="submit" name="contactsubmit" id="contactsubmit" value="{$form.form_submit}" class="button" onmouseover="this.className='button buttonrollover';" onmouseout="this.className='button'" /><br />
       </div>
