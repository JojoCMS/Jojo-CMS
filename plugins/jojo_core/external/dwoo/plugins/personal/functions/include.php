...
function Dwoo_Plugin_include(Dwoo $dwoo, $file, $cache_time = null, $cache_id = null, $compile_id = null, $data = '_root', $assign = null, array $rest = array())
{
	...
	if (Jojo::isMobile() && (strpos($file, '.mob.tpl')===false)) {
		$mobile_filename = str_replace('.tpl', '.mob.tpl', $file);
		foreach(Jojo::listPluginsReverse('templates/'.$mobile_filename) as $new_filename) {
		    $file = basename($new_filename);//todo - some templates legitimately work from a subfolder
		    break;
		}
	}
