...
{else}
<div class="{if $fd_size>=40}col-md-7{elseif $fd_size>10}col-md-4{else}col-md-2{/if}">
    <input class="form-control" type="text" name="fm_{$fd_field}" id="fm_{$fd_field}" size="{$size}" value="{if $value}{$value}{/if}" onchange="if (!validate(this.value,'integer')) alert('Please enter an integer value');" title="{$fd_help}" />{if $units}{$units}{/if}
