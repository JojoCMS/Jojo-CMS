<div class="{if $fd_size>=40}col-md-12{elseif $fd_size>10}col-md-8{else}col-md-4{/if} input-group{if $readonly} readonly{/if}">
{if $readonly}  <input type="hidden" name="fm_{$fd_field}" id="fm_{$fd_field}" size="{$size}" value="{if $value}{$value}{/if}" />{if $value}<span class="value">{$value}</span>{/if}{if $units}{$units}{/if}
{else}  <input class="form-control" type="text" name="fm_{$fd_field}" id="fm_{$fd_field}" size="{$size}" value="{if $value}{$value}{/if}" onchange="if (!validate(this.value,'integer')) alert('Please enter an integer value');" title="{$fd_help}" />
