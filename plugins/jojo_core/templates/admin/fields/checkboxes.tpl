{if $readonly}<input type="hidden" name="fm_{$fd_field}" id="fm_{$fd_field}" value="{$value}" />{* this has not been tested. Hidden fields may not exist *}
{/if}{assign var=first value=true}
...
{assign var=first value=false}{/foreach}
</div>