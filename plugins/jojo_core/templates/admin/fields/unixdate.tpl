{if $readonly != "yes"}
    <input type="text" size="10" name="fm_{$fd_field}" id="fm_{$fd_field}" {if $error != ""}class="error"{/if} value="{$formatteddate}"  title="{$fd_help}" onblur="magicDate(this)" onfocus="if (this.className != 'error') this.select()" {if $readonly == "yes"}readonly="readonly"{/if} />
