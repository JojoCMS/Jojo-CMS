{if $readonly}
    ...
    {$value}
{else}
    <input type="text" name="fm_{$fd_field}[1]" id="fm_{$fd_field}"  size="{$fd_size}" value="{$value}" onchange="if ((this.value!='') && (!validate(this.value,'email'))) alert('The email format is not valid');" title="{$fd_help}" />
    {if $confirm}<br />
