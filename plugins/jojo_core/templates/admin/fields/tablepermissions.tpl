    ...
    {foreach from=$groups key=group item=groupname}
        ...
        {foreach from=$_permOptions key=perm item=name}
            <td style="border: 1px solid black; text-align: center">
                <input type="checkbox" name="fm_{$fd_field}[{$group}.{$perm}]" value='1' {if isset($perms[$group]) && isset($perms[$group][$perm]) && $perms[$group][$perm]}checked="checked"{/if} {if $readonly}readonly="readonly"{/if} />
            </td>
        {/foreach}
        </tr>
    {/foreach}
