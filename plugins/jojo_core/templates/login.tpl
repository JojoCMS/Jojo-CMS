...
<div id="login-form">
  <form method="post" action="{if $issecure}{$SECUREURL}{else}{$SITEURL}{/if}/{$RELATIVE_URL}">
    ...
    <input type="text" name="username" tabindex=10 id="username" value="{if $username}{$username}{/if}" /><br />

    <label for="password">Password:</label>
    <input type="password" name="password" tabindex=20 id="password" value="{if $password}{$password}{/if}" title="Passwords are case-sensitive." /><br />

    <label for="remember" title="This option will log you in automatically from this computer.">Remember Password:</label>
    <input type="checkbox" name="remember" tabindex=30 id="remember" value="1" {if $remember=="1"} checked{/if} /><br />
