{include file="doctype.tpl"}<head>
    ...
    {/if}{if $jqueryhead || $isadmin}<script {if !$htmldoctype}type="text/javascript" {/if}src="{if $OPTIONS.googleajaxlibs == "yes"}http{if $issecure}s{/if}://ajax.googleapis.com/ajax/libs/jquery/{if $OPTIONS.jquery_version}{$OPTIONS.jquery_version}{else}1.7.2{/if}/jquery.min.js{else}{cycle values=$NEXTASSET}external/jquery/jquery-{if $OPTIONS.jquery_version}{$OPTIONS.jquery_version}{else}1.7.2{/if}.min.js{/if}"></script>
    ...
    <script type="text/javascript" src="{jojoAsset file="external/bootstrap/js/dropdown.js"}"></script>
    <script type="text/javascript" src="{jojoAsset file="external/bootstrap/js/tab.js"}"></script>
